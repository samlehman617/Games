...
function Laser(spos, angle) {
  ...
  // instead of creating its own separate object type because there is a large
  // overlap in terms of properties.
  Entity.call(this, spos.x, spos.y, 4);

  this.text = random(laserStr);
  ...
  this.render = function() {
    ...
    text(this.text, this.pos.x-20  , this.pos.y);
    //rotate(-PI/2);
    pop();
  }
  ...
  this.hits = function(asteroid) {
    ...
    if (dist2 <= asteroid.rmin2+30) {
      return true;
    }
    if (dist2 >= asteroid.rmax2+30) {
      return false;
    }
